...
test('Today recovery is read-only and canonical status owns liveness', () => {
  const today = fs.readFileSync(path.join(process.cwd(), 'app/[username]/page.tsx'), 'utf8')
  const migration = fs.readFileSync(path.join(process.cwd(), 'supabase/sql/add_player_workflow_status.sql'), 'utf8')
  const recoveryMigration = fs.readFileSync(path.join(process.cwd(), 'supabase/sql/fix_system_owned_workflow_recovery.sql'), 'utf8')
  const service = fs.readFileSync(path.join(process.cwd(), 'lib/player-workflow-status.ts'), 'utf8')
  const checkStatusStart = today.indexOf('async function checkGenerationStatus()')
  const checkStatusEnd = today.indexOf('function focusComposer()', checkStatusStart)
  const checkStatusBody = today.slice(checkStatusStart, checkStatusEnd)

  assert.ok(checkStatusStart >= 0)
  assert.match(today, /getPlayerWorkflowStatus/)
  assert.match(service, /get_player_workflow_status_v2/)
  ...
  assert.match(today, /nggak memulai atau mengulang proses AI/)
  assert.doesNotMatch(today, />RETRY</)
  assert.doesNotMatch(today, /async function retryGeneration/)
  assert.match(recoveryMigration, /create or replace function public\.get_player_workflow_status_v2/)
  assert.doesNotMatch(recoveryMigration.slice(recoveryMigration.indexOf('create or replace function public.get_player_workflow_status_v2'), recoveryMigration.indexOf('revoke all on function public.get_player_workflow_status_v2')), /request_progression_cycle|ensure_player_progression_session/)
  assert.match(recoveryMigration, /'turnOwner','system'/)
  assert.match(migration, /available_at <= v_now - interval '90 seconds'/)
  assert.match(migration, /lease_expires_at is null or v_job\.lease_expires_at <= v_now/)
  assert.match(migration, /v_eta_samples < 5/)
  assert.match(migration, /revoke all on function public\.resume_progression_cycle_operator\(uuid,date,text\) from public, anon, authenticated/)
  assert.match(migration, /grant execute on function public\.resume_progression_cycle_operator\(uuid,date,text\) to service_role/)
})
