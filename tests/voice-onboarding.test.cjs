...
test('player onboarding has a concise emotional arc without exposing architecture', () => {
  ...
  assert.match(page, /Gue lagi nyusun semua yang lo ceritain biar nyambung\./)
  assert.match(page, /Mungkin agak lebih lama dari biasanya, tapi tenang aja jawaban lo tetap kesimpan\./)
  assert.match(page, /PLAYER IDENTIFIED/)
  assert.match(page, /Mulai sekarang, System yang jaga arahnya\./)
  assert.match(page, /MASUK →/)
  assert.match(page, /Balas Superhuman…/)
  assert.match(page, /data-conversation-thread="onboarding"/)
