...
-- Daily Context gates the first quest, not the processing of newly supplied
-- clarification evidence. If there is pending progression work, allow the
-- worker to consume it; worker-v2 remains responsible for stopping at
-- awaiting_context before first quest generation when Daily Context is absent.
...
begin
  ...
  select exists(
    select 1 from public.daily_contexts where user_id=v_user_id and context_date=p_target_date
  ...
  if not v_has_quests and not v_has_daily_context and not v_has_pending_progression then
