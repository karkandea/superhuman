-- Queue + control plane for using an authenticated ChatGPT consumer browser session
-- as a provider behind Superhuman's existing provider-neutral AI orchestration.
--
-- This migration does NOT store ChatGPT credentials or browser cookies. The browser
-- worker owns its persistent browser profile outside the repository and only receives
-- bounded context after claiming a player-owned job.

create table if not exists public.ai_inference_jobs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.users(id) on delete cascade,
  operation text not null default 'progression_cycle' check (operation in ('progression_cycle')),
  ...
  worker_id text,
  provider_id text,
  provider_conversation_refs jsonb not null default '[]'::jsonb,
  ...
  constraint ai_inference_jobs_provider_refs_array check (jsonb_typeof(provider_conversation_refs) = 'array'),
  constraint ai_inference_jobs_result_summary_object check (jsonb_typeof(result_summary) = 'object'),
  unique (user_id, operation, target_date)
);

create index if not exists ai_inference_jobs_queue_idx
  on public.ai_inference_jobs(status, available_at, created_at);
create index if not exists ai_inference_jobs_user_date_idx
  on public.ai_inference_jobs(user_id, target_date desc, created_at desc);
...
create policy ai_inference_jobs_select_own
on public.ai_inference_jobs for select to authenticated
...
begin
  ...
  insert into public.ai_inference_jobs (user_id, target_date, status)
  values (v_user_id, p_target_date, case when v_has_quests then 'succeeded' else 'queued' end)
  returning * into v_job;
...
create or replace function public.heartbeat_ai_inference_job(
  p_job_id uuid,
  p_worker_id text,
  p_lease_seconds integer default 300
)
...
revoke execute on function public.schedule_ai_inference_retry(uuid, text, text, text, integer, text, jsonb)
  from public, anon, authenticated;
grant execute on function public.schedule_ai_inference_retry(uuid, text, text, text, integer, text, jsonb)
  to service_role;
...
begin
  ...
  if p_status not in ('succeeded', 'failed', 'blocked_auth') then
    raise exception 'Unsupported terminal inference status';
  end if;
  if p_provider_conversation_refs is null or jsonb_typeof(p_provider_conversation_refs) <> 'array' then
    raise exception 'provider conversation refs must be a JSON array';
  end if;
...
revoke execute on function public.complete_ai_inference_job(uuid, text, text, text, jsonb, jsonb, text, text)
  from public, anon, authenticated;
grant execute on function public.complete_ai_inference_job(uuid, text, text, text, jsonb, jsonb, text, text)
  to service_role;

-- Keep quest completion + quest-result feedback atomic so future generation always sees
-- a consistent outcome signal. This only operates on the authenticated player's quest.
...
create policy quest_results_delete_own on public.quest_results for delete to authenticated
using ((select auth.uid()) = user_id);
