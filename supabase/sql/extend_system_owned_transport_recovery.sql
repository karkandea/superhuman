-- Keep System-owned failures truthful and recover browser/transport terminal failures once.
-- Model/output recovery and browser/transport recovery use separate bounded budgets.
...
begin
  ...
  -- A terminal progression failure may receive one model/output recovery and one
  -- independent browser/transport recovery. Both are recent-only and preserve
  -- durable progression_run_steps, so recovery resumes instead of starting over.
  ...
  if found then
    ...
    v_is_transport_recovery := v_recovery_job.error_code in (
      'pre_submission_state_invalid',
      'composer_fill_timeout',
      'composer_fill_unverified',
      'composer_not_found',
