...
-- Preserve the existing progression-cycle retry/materiality behavior while adding one gate:
-- a first Daily Quest batch cannot be manually requested until the player confirms Daily Context.
-- Knowledge ingestion may still enqueue a worker before check-in; worker-v2 processes memory safely
-- and then stops at awaiting_context rather than generating quests.
