...
import { ManualInferencePendingError, ManualRelayProvider } from '../../lib/ai/manual-relay-provider.ts'
...
const WORKER_ID = process.env.SUPERHUMAN_MANUAL_WORKER_ID
  || process.env.SUPERHUMAN_WORKER_ID
  || `superhuman-manual-${hostname()}:${process.pid}:${randomUUID().slice(0, 8)}`
const POLL_MS = Number(process.env.SUPERHUMAN_MANUAL_WORKER_POLL_MS || process.env.SUPERHUMAN_WORKER_POLL_MS || 2500)
const LEASE_SECONDS = Number(process.env.SUPERHUMAN_MANUAL_WORKER_LEASE_SECONDS || process.env.SUPERHUMAN_WORKER_LEASE_SECONDS || 300)
...
const AI_STAGE_PAUSE_MS = Number(process.env.SUPERHUMAN_AI_STAGE_PAUSE_MS || 0)
const UNDERSTANDING_DELTA_VERSION = 'understanding-delta.v1'
const PROVIDER_ID = 'manual-relay'
...
async function pauseForOperator(client, job, turnId) {
  const { data, error } = await client.rpc('pause_ai_inference_job_for_operator', {
    p_job_id: job.id,
    p_worker_id: WORKER_ID,
    p_turn_id: turnId,
  })
  if (error) throw new Error(`pause inference job for operator: ${error.message}`)
  return normalizeRpcRow(data)
}
...
function classifyError(error) {
  ...
  if (/evidence-backed player signals|No player knowledge was retrieved|At least one knowledge entry/.test(message)) {
    return { code: 'insufficient_context', message, retryable: false }
  }
  if (/correlation mismatch|operation mismatch|schema version mismatch|malformed JSON|parseable JSON|sourceSignalIds|sourceKnowledgeEntryIds|targetUnderstandingId|outside retrieved context|outside current Player Brief|Understanding delta|Activity voice transcript|Materiality|materiality|System Interrupt|Interrupt action|interrupt plan|affectedQuestIds|urgency|recommendedAction|Quest Policy|Quest candidate|Quest selection|Quest portfolio|Progression Map|Progression Target|Player Response Model|Quest response review|strategic chain|strategic driver|feasibility|receptivity|executionContract|execution contract|effectiveness/.test(message)) {
    return { code: 'model_output_invalid', message, retryable: false }
  }
  if (/Player Brief is missing|Player brief changed before understanding delta persistence|Progression Map changed before Daily Quest reasoning|Progression Map changed during Daily Quest decision|Player Response Model changed before Daily Quest reasoning/.test(message)) {
    return { code: 'stale_player_brief', message, retryable: true }
  }
  if (/backlog did not drain|same knowledge batch repeated/.test(message)) {
    return { code: 'backlog_not_drained', message, retryable: false }
  }
  if (/timeout|fetch failed|network|connection|temporar/i.test(message)) {
    return { code: 'transient_transport_error', message, retryable: true }
  }
  return { code: 'inference_failed', message, retryable: false }
}

function errorClass(error) {
  if (error.code === 'transient_transport_error' || error.code === 'stale_player_brief') return 'transient'
...
async function scheduleRetry(client, job, error, refs) {
  const delaySeconds = Math.min(60, 2 ** Number(job.attempt_count || 1) * 3)
...
async function processJob(client, job) {
  const provider = new ManualRelayProvider(client, {
    jobId: job.id,
    userId: job.user_id,
  ...
  try {
    ...
    while (true) {
      ...
      if (AI_STAGE_PAUSE_MS > 0) await sleep(AI_STAGE_PAUSE_MS)
    }
    ...
    if (understandingStepStarted) await finishStep()
    ...
    if (shouldRefreshStrategicState) {
      if (knowledgeBatchCount > 0 && AI_STAGE_PAUSE_MS > 0) await sleep(AI_STAGE_PAUSE_MS)
      await startStep(
        'progression_map',
    ...
    if (hadDailyPlan) {
      ...
      if (materialityIds.length > 0) {
        if ((knowledgeBatchCount > 0 || shouldLearnFromResponses) && AI_STAGE_PAUSE_MS > 0) await sleep(AI_STAGE_PAUSE_MS)
    ...
    if (!hadDailyPlan) {
      ...
      await finishStep({
        ...
        providerId: generated.requestId ? PROVIDER_ID : undefined,
    ...
    console.log(`[job ${job.id}] succeeded through manual relay: knowledge=${processedKnowledgeCount}/${knowledgeBatchCount} batches; deltaActions=${understandingDeltaActionCount}; brief=v${latestPlayerBriefVersion ?? 'unchanged'}; map=v${progressionMapVersion ?? 'none'}; responseModel=v${playerResponseModelVersion ?? 'none'}; plan=${generated.source}/${generated.quests.length} quests`)
  } catch (rawError) {
    if (rawError instanceof ManualInferencePendingError || rawError?.code === 'manual_inference_pending') {
      const turnId = rawError.turnId
      await pauseForOperator(client, job, turnId)
      console.log(`[job ${job.id}] waiting for operator on manual inference turn ${turnId} (${rawError.operation || 'unknown'})`)
      return
    }
    ...
    const mayRetry = error.retryable && Number(job.attempt_count || 0) < Number(job.max_attempts || 3)
    if (mayRetry) {
...
async function main() {
  ...
  console.log(`Superhuman manual relay worker online as ${WORKER_ID}`)
  console.log(`Provider=${PROVIDER_ID}; no browser/headless ChatGPT automation is used.`)
