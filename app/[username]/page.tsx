  ...
  return (
    <div style={{ minHeight: '100dvh', background: S.bg, color: S.ink, fontFamily: '"IBM Plex Sans", sans-serif', paddingBottom: 72 }}>
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '0 18px' }}>
        <header style={{ padding: '30px 0 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div>
              ...
              <div style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 11, color: S.muted, marginTop: 5 }}>PLAYER · {username.toUpperCase()}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <Link href={`/${encodeURIComponent(username)}/vault`} style={{ color: S.gold, fontFamily: '"IBM Plex Mono", monospace', fontSize: 10, textDecoration: 'none', letterSpacing: '.08em' }}>LIFE VAULT →</Link>
              <Link href={`/${encodeURIComponent(username)}/history`} style={{ color: S.muted, fontFamily: '"IBM Plex Mono", monospace', fontSize: 10, textDecoration: 'none', letterSpacing: '.08em' }}>HISTORY →</Link>
            </div>
          </div>

          <div style={{ marginTop: 26 }}>
            <div style={{ fontFamily: '"IBM Plex Mono", monospace', color: S.muted, fontSize: 11 }}>{new Date().toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</div>
            <h1 style={{ fontFamily: '"Space Grotesk", sans-serif', fontSize: 'clamp(30px,8vw,42px)', lineHeight: 1, letterSpacing: '-.04em', margin: '8px 0 0' }}>Daily Quest</h1>
            <p style={{ color: S.muted, fontSize: 13, lineHeight: 1.55, margin: '10px 0 0', maxWidth: 500 }}>
              {questReady ? 'Fokus hari ini stabil by default. Kalau situasi penting berubah, System akan melakukan interrupt secara eksplisit — bukan reshuffle diam-diam.' : 'Daily Quest dibuat otomatis dari apa yang System pahami tentang hidup, tujuan, dan hambatan lo.'}
            </p>
          </div>

          {!questReady && <SystemEmptyState username={username} generationStatus={generationStatus} needsContext={needsContext} generationFailed={generationFailed || emptyAfterSuccess} transportInterrupted={transportInterrupted} systemPaused={systemPaused} onRetry={() => { void retryGeneration() }} />}
          ...
          {questReady && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginTop: 22 }}>
                <Stat value={`${pct}%`} label="PROGRESS" /><Stat value={`${xpEarned}/${xpTotal}`} label="XP" /><Stat value={`${streak}`} label="STREAK" />
              </div>
              <div style={{ marginTop: 12, height: 6, background: '#1c222c', borderRadius: 99, overflow: 'hidden' }}><div style={{ width: `${pct}%`, height: '100%', background: `linear-gradient(90deg,${S.amber},${S.gold})`, transition: 'width 400ms ease' }} /></div>
              ...
              <div style={{ height: 14, marginTop: 8, fontFamily: '"IBM Plex Mono", monospace', fontSize: 10, color: saveStatus === 'saved' ? S.amber : S.muted, opacity: saveStatus === 'idle' ? 0 : 1 }}>{saveStatus === 'saving' ? 'SYNCING...' : saveStatus === 'saved' ? '✓ QUEST STATE SAVED' : ''}</div>
            </>
          )}
        </header>
        ...
        {quests.length > 0 && (
          <>
            <div style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: '2px 0 14px' }}>
              {(['semua', ...CATEGORY_ORDER] as const).map(category => <button key={category} onClick={() => setFilter(category)} style={{ flexShrink: 0, border: filter === category ? 'none' : `1px solid ${S.line}`, background: filter === category ? S.amber : 'transparent', color: filter === category ? S.bg : S.muted, borderRadius: 99, padding: '8px 13px', fontFamily: '"IBM Plex Mono", monospace', fontSize: 10, fontWeight: 600, cursor: 'pointer' }}>{category === 'semua' ? 'SEMUA' : CATEGORY_LABEL[category].toUpperCase()}</button>)}
            </div>
            <main>
              {visibleCategories.map(category => {
                const categoryQuests = quests.filter(quest => quest.category === category)
                if (!categoryQuests.length) return null
                return <section key={category} style={{ marginTop: 16 }}>
                  <div style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 10, color: S.muted, letterSpacing: '.12em', margin: '0 2px 8px' }}>{CATEGORY_LABEL[category].toUpperCase()}</div>
                  <div style={{ background: S.panel, border: `1px solid ${S.line}`, borderRadius: 18, overflow: 'hidden' }}>
                    {categoryQuests.map((quest, index) => {
                      ...
                      return <div key={quest.id} onClick={() => toggle(quest.id)} role="checkbox" aria-checked={done} tabIndex={0} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); toggle(quest.id) } }} style={{ display: 'flex', gap: 13, padding: '15px 16px', borderTop: index ? `1px solid ${S.line}` : 'none', cursor: 'pointer', background: done ? 'rgba(246,178,75,.035)' : 'transparent' }}>
                        <span style={{ width: 22, height: 22, flexShrink: 0, borderRadius: 7, border: done ? 'none' : '1.5px solid #39414e', display: 'grid', placeItems: 'center', background: done ? `linear-gradient(135deg,${S.amber},${S.gold})` : 'transparent', boxShadow: done ? '0 0 14px rgba(246,178,75,.4)' : 'none', marginTop: 2 }}>{done ? '✓' : ''}</span>
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
                            <span style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 9, color: quest.kind === 'main' ? S.amber : S.muted, letterSpacing: '.1em' }}>{questKindLabel[quest.kind]}</span>
                          ...
                          <div style={{ marginTop: 5, fontSize: 14.5, lineHeight: 1.45, color: done ? S.muted : S.ink, textDecoration: done ? 'line-through' : 'none' }}>{quest.title}</div>
                          {quest.rationale && <div style={{ marginTop: 6, color: S.muted, fontSize: 11, lineHeight: 1.45 }}>{quest.rationale}</div>}
                        </div>
                      </div>
                    })}
                  </div>
        ...
        {adjustedQuests.length > 0 && <section style={{ marginTop: 28 }}>
          ...
          <div style={{ border: `1px solid ${S.line}`, borderRadius: 14, background: S.panel2, overflow: 'hidden' }}>
            {adjustedQuests.map((quest, index) => <div key={quest.id} style={{ padding: '11px 13px', borderTop: index ? `1px solid ${S.line}` : 'none' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'baseline' }}><div style={{ color: S.muted, fontSize: 12.5, textDecoration: quest.status === 'replaced' ? 'line-through' : 'none' }}>{quest.title}</div><div style={{ fontFamily: '"IBM Plex Mono", monospace', color: S.amber, fontSize: 8, letterSpacing: '.08em' }}>{quest.status.toUpperCase()}</div></div>
        ...
        <footer style={{ padding: '32px 0 10px', textAlign: 'center' }}>
          <div style={{ color: S.muted, fontFamily: '"IBM Plex Mono", monospace', fontSize: 9, lineHeight: 1.6 }}>QUEST AUTHORING IS SYSTEM-OWNED<br />{questReady ? 'stable by default · explicit interrupts when material context changes' : 'add life context and let the System decide what matters next'}</div>
          <Link href="/" style={{ display: 'inline-block', marginTop: 16, color: S.muted, textDecoration: 'none', fontFamily: '"IBM Plex Mono", monospace', fontSize: 10 }}>← SWITCH PLAYER</Link>
          {generationErrorMessage && process.env.NODE_ENV !== 'production' && <div style={{ marginTop: 12, color: S.red, fontFamily: '"IBM Plex Mono", monospace', fontSize: 9 }}>DEBUG · {generationErrorCode ?? 'unknown'} · {generationErrorMessage}</div>}
        </footer>
      </div>
    </div>
  )
...
function SystemEmptyState({ username, generationStatus, needsContext, generationFailed, transportInterrupted, systemPaused, onRetry }: { username: string; generationStatus: AiInferenceJobStatus | 'idle'; needsContext: boolean; generationFailed: boolean; transportInterrupted: boolean; systemPaused: boolean; onRetry: () => void }) {
  ...
  if (queued) { eyebrow = 'UPDATE RECEIVED'; title = 'SYSTEM IS PREPARING YOUR PROGRESSION'; body = 'Konteks lo sudah masuk. System akan memprosesnya dan Daily Quest akan muncul otomatis setelah progression siap.' }
  else if (running) { eyebrow = 'UNDERSTANDING PLAYER'; title = 'SYSTEM IS ANALYZING YOUR CONTEXT'; body = 'System sedang menghubungkan update terbaru dengan tujuan, hambatan, dan progression lo. Daily Quest akan muncul otomatis setelah selesai.' }
  else if (systemPaused) { eyebrow = 'SYSTEM TEMPORARILY PAUSED'; title = 'YOUR CONTEXT IS SAFE'; body = 'System belum bisa melanjutkan processing saat ini. Update lo tetap tersimpan dan tidak perlu dimasukkan ulang.' }
  else if (transportInterrupted) { eyebrow = 'SYSTEM TEMPORARILY INTERRUPTED'; title = 'YOUR CONTEXT IS SAFE'; body = 'Koneksi ke reasoning engine sempat terinterupsi. Tidak ada context yang hilang. Tunggu sebentar lalu coba lagi — lo tidak perlu menulis ulang atau menambah context.' }
  else if (generationFailed) { eyebrow = 'SYSTEM COULD NOT FINISH'; title = 'YOUR CONTEXT IS STILL SAFE'; body = 'System belum berhasil menyelesaikan progression kali ini. Context lo tetap tersimpan; coba proses ulang tanpa memasukkan data yang sama lagi.' }
  const canRetry = generationFailed || transportInterrupted
  return <div style={{ marginTop: 20, background: S.panel, border: `1px solid ${S.line}`, borderRadius: 18, padding: '18px 16px' }}>
    ...
    {needsContext && !queued && !running && !systemPaused && <Link href={`/${encodeURIComponent(username)}/vault`} style={{ display: 'block', marginTop: 16, borderRadius: 10, padding: '12px 16px', textAlign: 'center', background: S.amber, color: S.bg, textDecoration: 'none', fontFamily: '"IBM Plex Mono", monospace', fontWeight: 700, fontSize: 11, letterSpacing: '.08em' }}>ADD TO LIFE VAULT →</Link>}
    {canRetry && <button type="button" onClick={onRetry} style={{ width: '100%', marginTop: 16, border: 'none', borderRadius: 10, padding: '12px 16px', background: S.amber, color: S.bg, fontFamily: '"IBM Plex Mono", monospace', fontWeight: 700, fontSize: 11, letterSpacing: '.08em', cursor: 'pointer' }}>TRY AGAIN</button>}
  </div>
}

function Stat({ value, label }: { value: string; label: string }) {
  return <div style={{ background: S.panel, border: `1px solid ${S.line}`, borderRadius: 14, padding: '12px 10px' }}><div style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 20, fontWeight: 700, color: S.gold }}>{value}</div><div style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 8.5, color: S.muted, letterSpacing: '.1em', marginTop: 3 }}>{label}</div></div>
}
